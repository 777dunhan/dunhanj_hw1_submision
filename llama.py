...
class Attention(nn.Module):
    ...
    def compute_query_key_value_scores(self,
        ...
        # todo
        # raise NotImplementedError
        return F.scaled_dot_product_attention(query, key, value, dropout_p=self.attn_dropout.p, is_causal=False)
...
class LlamaLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        # todo
        # raise NotImplementedError
        x = x + self.attention(self.attention_norm(x)) # residual connection with self-attention
        x = x + self.feed_forward(self.ffn_norm(x)) # residual connection with feed-forward network
        return x
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0, top_p=0.9):
        ...
        for _ in range(max_new_tokens):
            ...
            if temperature == 0.0:
                ...
                '''
                sorted_logits, sorted_indices = torch.sort(F.softmax(logits / temperature, dim=-1), descending=True) # sort the logits
                mask = torch.cumsum(sorted_logits, dim=-1) > top_p # cumulative probabilities
                mask[:, 0] = False
                sorted_logits[mask] = 0 # mask the logits
                idx_next = torch.multinomial(sorted_logits / sorted_logits.sum(dim=-1, keepdim=True), 1)  # Sample from the filtered distribution (normalized prob)
                idx_next = sorted_indices.gather(dim=-1, index=idx_next) # convert back to original indices
